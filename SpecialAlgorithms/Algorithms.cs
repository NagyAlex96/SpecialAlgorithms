...
namespace SpecialAlgorithms
{
    public class Algorithms
    {
        ...
        private static int MaximumFelezoKereses<T>(IEnumerable<T> inputArray, int bal, int jobb) where T : IComparable<T>
        {
            if (bal == jobb)
            {
                return bal;
            ...
            else
            {
                ...
                if (inputArray.ElementAt(balmax).CompareTo(inputArray.ElementAt(jobbmax)) > 0)
                {
                    return balmax;
                }
                else
